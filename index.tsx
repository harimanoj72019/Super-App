...
function App() {
  ...
  const [savedArtifacts, setSavedArtifacts] = useState<SavedArtifact[]>(() => {
      const saved = localStorage.getItem('flash_ui_saved');
  ...
  useEffect(() => {
      localStorage.setItem('flash_ui_saved', JSON.stringify(savedArtifacts));
  }, [savedArtifacts]);
...
\`{ "name": "Persona Name", "html": "..." }\`
  ...
  const handleSaveArtifact = () => {
    ...
    if (savedArtifacts.some(a => a.id === artifact.id)) {
        // Simple toggle off if we want to remove? For now, let's just avoid dups
        return;
    }
    ...
    setSavedArtifacts(prev => [saved, ...prev]);
  };
  ...
  const handleShowLibrary = () => {
      setDrawerState({ isOpen: true, mode: 'library', title: 'My Library', data: null });
  };

  const handleRestoreFromLibrary = (saved: SavedArtifact) => {
...
Return ONLY RAW HTML. No markdown fences.
  ...
  const isCurrentFocusedSaved = currentSession && focusedArtifactIndex !== null && 
      savedArtifacts.some(a => a.id === currentSession.artifacts[focusedArtifactIndex].id);

  return (
    <>
        <div className={`top-nav ${hasStarted ? 'mini' : ''}`}>
            <a href="https://x.com/ammaar" target="_blank" rel="noreferrer" className="creator-credit">
                created by @ammaar
            </a>
            {savedArtifacts.length > 0 && (
                <button className="library-toggle-btn" onClick={handleShowLibrary} aria-label="Library">
                    <LibraryIcon /> Library
                </button>
            )}
        </div>
                ...
                <div className="sexy-grid">
                    {savedArtifacts.length === 0 && (
                        <div className="empty-library">
                            <BookmarkIcon />
                            <p>Your library is empty. Save components to see them here.</p>
                        </div>
                    )}
                    {savedArtifacts.map((s, i) => (
                         <div key={s.id} className="sexy-card library-item" onClick={() => handleRestoreFromLibrary(s)}>
                             <div className="sexy-preview">
                                 <iframe srcDoc={s.html} title={s.styleName} sandbox="allow-scripts allow-same-origin" />
                                 <button className="delete-btn" onClick={(e) => handleRemoveSaved(s.id, e)}>
                                     <TrashIcon />
                                 </button>
                             </div>
                             <div className="sexy-label">
                                 <div className="item-prompt">{s.prompt}</div>
                                 <div className="item-style">{s.styleName}</div>
                             </div>
                         </div>
                    ))}
                </div>
        ...
        <div className="immersive-app">
                 ...
                 <div className={`empty-state ${hasStarted ? 'fade-out' : ''}`}>
                     <div className="empty-content">
                         ...
                         <div className="landing-actions">
                             ...
                             {savedArtifacts.length > 0 && (
                                 <button className="secondary-button" onClick={handleShowLibrary}>
            ...
            <div className={`action-bar ${focusedArtifactIndex !== null ? 'visible' : ''}`}>
                 ...
                 <div className="action-buttons">
                    ...
                    <button onClick={handleSaveArtifact} className={isCurrentFocusedSaved ? 'saved' : ''}>
                        <BookmarkIcon /> {isCurrentFocusedSaved ? 'Saved' : 'Save'}
                    </button>
